...
/*
 *	/home/wcl/frappe-bench/apps/frappe/frappe/public/js/lib/jquery/jquery-ui.min.js
 */
...
/*
 *	/home/wcl/frappe-bench/apps/frappe/frappe/public/js/lib/Sortable.min.js
 */
...
/*
 *	/home/wcl/frappe-bench/apps/frappe/frappe/public/js/lib/tag-it.min.js
 */
...
/*
 *	/home/wcl/frappe-bench/apps/frappe/frappe/public/js/lib/bootstrap.min.js
 */
/*!
...
return NProgress;});;
/*
 *	/home/wcl/frappe-bench/apps/frappe/frappe/public/js/lib/moment/moment.min.js
 */
//! moment.js
//! version : 2.6.0
...
/*
 *	/home/wcl/frappe-bench/apps/frappe/frappe/public/js/lib/moment/moment-timezone.min.js
 */
...
this.get_main_icon(this.icon);this.main=this.wrapper.find(".layout-main-section");this.sidebar=this.wrapper.find(".layout-side-section");this.indicator=this.wrapper.find(".indicator");this.page_actions=this.wrapper.find(".page-actions");this.btn_primary=this.page_actions.find(".primary-action");this.btn_secondary=this.page_actions.find(".btn-secondary");this.menu=this.page_actions.find(".menu-btn-group .dropdown-menu");this.menu_btn_group=this.page_actions.find(".menu-btn-group");this.actions=this.page_actions.find(".actions-btn-group .dropdown-menu");this.actions_btn_group=this.page_actions.find(".actions-btn-group");this.page_form=$('<div class="page-form row hide"></div>').prependTo(this.main);this.icon_group=this.page_actions.find(".page-icon-group");},set_indicator:function(label,color){this.clear_indicator().removeClass("hide").html(label).addClass(color);},add_action_icon:function(icon,click){return $('<a class="text-muted"><i class="'+icon+'"></i></a>').appendTo(this.icon_group.removeClass("hide")).click(click);},clear_indicator:function(){return this.indicator.removeClass().addClass("indicator hide");},get_icon_label:function(icon,label){return'<i class="visible-xs '+icon+'"></i><span class="hidden-xs">'+label+'</span>'},set_action:function(btn,opts){if(opts.icon){opts.label=this.get_icon_label(opts.icon,opts.label);}
...
if(this.render_input){this.refresh();}},make:function(){this.make_wrapper();this.$wrapper.addClass("ui-front").attr("data-fieldtype",this.df.fieldtype);this.wrapper=this.$wrapper.get(0);this.wrapper.fieldobj=this;},make_wrapper:function(){this.$wrapper=$("<div class='frappe-control'></div>").appendTo(this.parent);},toggle:function(show){this.df.hidden=show?0:1;this.refresh();},get_status:function(explain){var status=frappe.perm.get_field_display_status(this.df,frappe.model.get_doc(this.doctype,this.docname),this.perm||(this.frm&&this.frm.perm),explain);if(this.doctype&&status==="Read"&&is_null(frappe.model.get_value(this.doctype,this.docname,this.df.fieldname))&&!in_list(["HTML"],this.df.fieldtype)){if(explain)console.log("By Hide Read-only, null fields: None");status="None";}
...
if(me.disp_status=="Write"){me.disp_area&&$(me.disp_area).toggle(false);$(me.input_area).toggle(true);$(me.input_area).find("input").prop("disabled",false);if(!me.has_input){me.make_input();if(me.df.on_make){me.df.on_make(me);}};if(me.doctype&&me.docname){me.set_input(me.value);}else{me.set_input();}}else{$(me.input_area).toggle(false);$(me.input_area).find("input").prop("disabled",true);if(me.disp_area){me.set_disp_area();$(me.disp_area).toggle(true);}}
...
frappe.set_route("Form",me.get_options(),value);});if(this.only_input)this.$input_area.find(".link-btn").remove();},open_advanced_search:function(){var doctype=this.get_options();if(!doctype)return;new frappe.ui.form.LinkSelector({doctype:doctype,target:this,txt:this.get_value()});},new_doc:function(){var doctype=this.get_options();if(!doctype)return;if(this.frm){this.frm.new_doc(doctype,this,{"name_field":this.get_value()});}else{new_doc(doctype,{"name_field":this.get_value()});}},setup_autocomplete:function(){var me=this;this.$input.on("blur",function(){if(me.selected){me.selected=false;return;}
...
return arr;},all:function(lst){for(var i=0,l=lst.length;i<l;i++){if(!lst[i]){return false;}}
...
if(breadcrumbs.module&&breadcrumbs.module!="Desk"){if(in_list(["Core","Email","Custom","Workflow"],breadcrumbs.module))
breadcrumbs.module="Setup";var module_info=frappe.get_module(breadcrumbs.module),icon=module_info&&module_info.icon,label=module_info?module_info.label:breadcrumbs.module;if(module_info){$(repl('<li><a href="#Module/%(module)s">%(label)s</a></li>',{module:breadcrumbs.module,label:__(label)})).appendTo($breadcrumbs);}}
if(breadcrumbs.doctype&&frappe.get_route()[0]==="Form"){$(repl('<li><a href="#List/%(doctype)s">%(label)s</a></li>',{doctype:breadcrumbs.doctype,label:__(breadcrumbs.doctype)})).appendTo($breadcrumbs);}
...
$(this).val('');return false;}};var render_item=function(ul,d){var html="<span>"+__(d.value)+"</span>";if(d.description&&d.value!==d.description){html+='<br><span class="text-muted">'+__(d.description)+'</span>';}
...
msgprint(txt,"Search Help");}});},make_page_title_map:function(){frappe.search.pages={};$.each(frappe.boot.page_info,function(name,p){frappe.search.pages[p.title]=p;p.name=name;});},setup_recent:function(){var recent=JSON.parse(frappe.boot.user.recent||"[]")||[];frappe.search.recent={};for(var i=0,l=recent.length;i<l;i++){var d=recent[i];if(!frappe.search.recent[d[0]]){frappe.search.recent[d[0]]=[];}
...
+(signature?("<br><br>"+signature):"");if(last_email){fields.content.set_input(reply
+"<!-- original-reply --><br>"
+'<blockquote>'+'<p>'+__("On {0}, {1} wrote:",[frappe.datetime.global_date_format(last_email.creation),last_email.comment_by])+'</p>'+
frappe.markdown(last_email.comment)+'<blockquote>');}else{fields.content.set_input(reply);}},setup_autosuggest:function(){var me=this;function split(val){return val.split(/,\s*/);}
